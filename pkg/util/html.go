...
func GetHtmlMatchDataLinkTitle(data, matchTag string, limit int) []HtmlLinkInfo {
	res := HtmlMatchDataLinkTitle(data)
...
func HtmlMatchDataLink(data string) []HtmlLinkInfo {
	pat := regexp.MustCompile(`<li(.|\n)*?data-link="(.*?)"(.|\n)*?>`)
	res := pat.FindAllStringSubmatch(data, -1)
	titles := make([]HtmlLinkInfo, 0)
	for _, v := range res {
		if len(v) < 4 {
...
func HtmlMatchDataLinkTitle(data string) []HtmlLinkInfo {
	pat := regexp.MustCompile(`(<li [\s\S]*?data-link="(.*?)"[\s\S]*?title="(.*?)"[\s\S]*?>)`)
	res := pat.FindAllStringSubmatch(data, -1)
